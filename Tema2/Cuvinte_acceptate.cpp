...
void Automat::genereazaCuvintele(string cuvant){
    for(int i = 0; i < this->cardAlfabet; i++){
        if(cuvant.size() < this->lungimeCuvinte){
            if(cuvant.size() < this->alfabet[i] - 1){
                genereazaCuvintele(cuvant + alfabet[i]);
                if(verificaAcceptare(cuvant + alfabet[i]) == 1){
                    this->ok = 1;
                    cout << cuvant + alfabet[i] << endl;
                }
            }
        }
    }
    ...
    for(int i = 0; i < this->tranzitii; i++)
        if(stare == this->matriceAutomat[i][0] && this->matriceAutomat[i][2] == ' '){
            ...
            if(ok == 1){
                ...
                cautaLambdaInchideri(inchideri, this->matriceAutomat[i][1]);
            }
        }
}
...
void Automat::seteazaLambdaInchideri(){
    ...
    for(int i = 0; i < this->nrStari; i++){
        ...
        cautaLambdaInchideri(this->lambdaInchideri[i], stare);
    }
}

// Verifica daca un cuvant este acceptat de AFN-lambda
bool Automat::verificaAcceptare(string cuvant){
    int index = int(this->stareInitiala - 48);
    string stari = this->lambdaInchideri[0];   // Inchideri lambda pentru starea initiala
    ...
    for(int i = 0; i < cuvant.size(); i++){
        ...
        for(int j = 0; j < stari.size(); j++){
            for(int k = 0; k < this->tranzitii; k++)
                if(this->matriceAutomat[k][0] == stari[j] && this->matriceAutomat[k][2] == cuvant[i]){
                    bool ok = 1;
                    for(int l = 0; l < stariCurente.size() && ok; l++)
        ...
        for(int i = 0; i < stari.size(); i++){
            string temp = "";
            temp += stari[i];
            cautaLambdaInchideri(temp, stari[i]);
            for(int j = 0; j < temp.size(); j++){
                bool ok = 1;
                for(int k = 0; k < stariCurente.size() && ok; k++)
    ...
    for(int i = 0; i < stari.size(); i++)
        for(int j = 0; j < this->nrStariFinale; j++)
